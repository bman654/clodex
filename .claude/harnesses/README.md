...
## Patcher / PATCH 5, PATCH 6, PATCH 10 and PATCH 11
...
| `cc282-patch11-catalog-picker-over-real-bundles` | PATCH 11's anchor binds the /model picker's ENTRY POINT once per bundle, identified from its own content (the custom-model env option Claude Code appends to that same array), and the identifier after its `??` is the very builder PATCH 5 patches — which is what makes "PATCH 5's rows are unreachable when a catalog is served" a fact about the bundle rather than a story. It then EXECUTES the patched entry point on both paths and asserts the aliases come back while the legacy builder is called on the fallback path and not on the served one. Last verified against 39 bundles, 2.1.257 → 2.1.282, including all eight platform builds of 2.1.257, 2.1.260 and 2.1.282. Its legacy stub returns no alias rows, so it cannot see duplicates on the fallback path; `tests/patch-catalog-picker.test.ts` covers that. Copy this one when a release moves the picker's entry point. |
