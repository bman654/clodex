...
| `patch11-autocompact-execute-real-resolver` | Extracts the *patched* auto-compact resolver out of a real bundle and **executes** it: per-model window, clamping to the model maximum, precedence under an explicit setting and the environment override, and that a prototype key never resolves through the baked table. Binds free identifiers through a `with` + Proxy scope, so a name it failed to anticipate throws rather than reaching a real global and passing vacuously. |
