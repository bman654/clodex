...
Releases are managed by release-please from conventional commits. Write conventional commits, then push or merge them to `main`; release-please maintains a release PR that accumulates the generated changelog and `package.json` version bump. Merging that release PR creates the `vX.Y.Z` tag and GitHub Release, then the same workflow installs with pnpm on Node 24, runs typecheck + tests + build, and publishes to npm via trusted publishing (OIDC, `id-token: write` — no npm secret anywhere). **Do NOT run `npm publish` locally.** Manually pushing a `v*` tag no longer publishes anything; `.github/workflows/release-please.yml` is the only release path.

The release and commitlint CI jobs are guarded by `if: vars.CLODEX_PUBLISH_ENABLED == 'true'` and therefore remain inert while this branch lives in relay-ai. After migrating clodex to its dedicated repo: (1) create the `clodex` package on npm once with a placeholder `0.0.0` publish (trusted publishing cannot do a package's first publish), (2) on npmjs.com configure the package's Trusted Publisher: GitHub Actions, the new repo's owner/name, workflow filename `release-please.yml`, allowed action "publish", (3) set repository variable `CLODEX_PUBLISH_ENABLED=true`. No npm token secret is needed. The release workflow deliberately publishes in the release-please job because a tag created by the default `GITHUB_TOKEN` does not trigger another workflow.
