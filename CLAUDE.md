...
- `startProxyCatalog(routes, startingAliasId, debug, inferenceLogPath?, debugLogPath?, webSocketDiagnosticsLogPath?, modelAliases?)` — multi-route catalog proxy for switch-menu sessions; the optional log path records upstream response failures. `modelAliases` registers short alias names (e.g. `sol`) as resolvable request model ids; the HTTP-proxy MITM layer forwards request bodies **unrewritten** so responses echo the exact model id the client sent — Claude Code resolves context windows from the response `model` field, and substituting the canonical `relay:{provider}:{model}` id there made patched/alias ids miss their window config (auto-compact never fired → "Prompt is too long" agent deaths).
