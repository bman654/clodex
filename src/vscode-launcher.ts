...
export function successReport(result: Extract<InstallVscodeLauncherResult, { ok: true }>): string[] {
  return [
    ...
    '',
    'Note: clodex-claude\'s substitution of your clodex-patched install (what puts clodex models',
    'in the extension\'s model picker on macOS/Linux) is not enabled on Windows. Here it runs the',
    'bundled binary the extension hands it, so the picker stays as it was; you get routing.',
    '',
